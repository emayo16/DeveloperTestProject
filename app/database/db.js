...
r.connect( dbconfig, function(err, conn) {
    ...
    r.db(dbconfig.db).table("tracks").indexWait("createdAt").run(conn).then(function(err, result) {
        console.log("tracks table is created.");
    }).error(function(err) {
        // The database/table/index do not exist. Create them.
        console.log("before dbcreate: " + err);
        r.dbCreate(dbconfig.db).run(conn).finally(function() {
            console.log("before tableCreate");
            if(r.db(dbconfig.db).tableList().contains("tracks")){
                console.log("inside cotnains");
                return r.db(dbconfig.db).tableCreate("tracks").run(conn)
            }
        }).finally(function() {
                r.db(dbconfig.db).table("tracks").indexCreate("createdAt").run(conn); 
    ...
    var insertTestData = process.argv[2];
